...
Env vars (used by container deployments):
  DB_PATH  where to put the SQLite file        (default: ./py_sse_chat.db)
...
# DB path: defaults to cwd for local dev; in production set DB_PATH to a
# persistent volume mount (e.g. /data/py_sse_chat.db on Coolify).
DB_PATH = os.environ.get("DB_PATH", "py_sse_chat.db")

def db():
    "Per-thread connection to the SQLite DB at $DB_PATH."
...
if __name__ == "__main__":
    # In a container, must bind 0.0.0.0 so the proxy (Traefik) can reach
    # us. Override per-environment via $HOST and $PORT.
    serve(
        ROUTES,
        host=os.environ.get("HOST", "127.0.0.1"),
        port=int(os.environ.get("PORT", "8000")),
        before_hooks=[attach_user],
    )
