...
# Stride: how many sampler ticks between chart-point updates. The chart
# deque only gets a new point every `stride` ticks. Between those, the
# polyline string is byte-identical → brotli compresses it for free.
...
_state = {
    ...
    # Running EMA value per (window, core). The actual filter state —
    # advanced every sampler tick, not stored as a history.
    "ema_state": {label: [] for label, _ in WINDOWS},

    # Chart-resolution per (window, core): deque of exactly CHART_POINTS
    # values. Updated only every WINDOW_STRIDE[window] ticks. This is
    # what gets rendered. Between updates: byte-identical → brotli wins.
    "chart_points": {label: [] for label, _ in WINDOWS},

    # Per-window tick counter for deciding when to push a chart point.
...
# ─── sampler ──────────────────────────────────────────────────────────
# Every tick:
#   1. Advance the EMA filter state per (window, core).
#   2. Increment the per-window chart_tick counter.
#   3. When chart_tick reaches stride, append current EMA value to the
#      chart_points deque and reset counter.
# Between these "chart updates" the rendered polyline is byte-identical.
...
def _advance_per_tick(per_core_values):
    """Advance EMA + maybe push a chart point per window."""
    for label, _ in WINDOWS:
    ...
    while True:
        ...
        with _state_lock:
            ...
            if not initialized and per_core:
                ...
                # Seed each EMA at its own core's value (no cold ramp).
                for label, _ in WINDOWS:
...
def snapshot(window):
    """Snapshot for a specific window. Returns the chart-resolution
    deque (exactly CHART_POINTS samples) for each core."""
    with _state_lock:
...
# ─── CPU chart with legend ───────────────────────────────────────────
# `chart` is already at CHART_POINTS resolution and only updates every
# `stride` sampler ticks. Between updates, this entire SVG renders to
# byte-identical output → brotli reduces it to a back-reference.
...
def cpu_chart_with_legend(snap, window):
    ...
    for i in range(n_cores):
        ...
        core_groups.append(
            h.g({"id": f"cpu-line-c{i}",
                 "style": f"--fg: 0.85; --hue-shift: {hue}",
    ...
    return h.div({"id": "cpu-chart-card", "class": "card stage column"},
        h.div({"class": "spread"},
...
def stat_card(card_id, label, primary, tag_value=None, tag_kind="suc",
              extra=None, hue_shift=None):
    style = f"--hue-shift: {hue_shift}" if hue_shift is not None else None
    attrs = {"class": "card stage column", "id": card_id}
    if style:
        attrs["style"] = style
    children = [
        h.div({"class": "spread"},
...
def stats_row(snap, window):
    ...
    return h.div({"class": "grid", "style": "--grid-min: 11rem", "id": "stats-grid"},
        stat_card(
            "card-memory", "memory",
        ...
        stat_card(
            ...
            hue_shift=HUE_DISK),
        h.div({"class": "card stage column", "id": "card-net-rx",
        ...
        h.div({"class": "card stage column", "id": "card-net-tx",
...
def dashboard_inner(snap, viewer_count, mode, window):
    return [
        stats_row(snap, window),
        cpu_chart_with_legend(snap, window),
        h.div({"class": "card stage column", "id": "card-processes"},
            h.span({"style": "--type: -2; --fg: -0.5; text-transform: uppercase"},
...
def dashboard_live(snap, count, window):
    return h.div({"id": "dashboard", "class": "column"},
                 *dashboard_inner(snap, count, "live", window))


def dashboard_poll(snap, count, interval_ms, window):
    return h.div(
        {"id": "dashboard", "class": "column",
         f"data-on-interval__duration.{interval_ms}ms":
             f"@get('/dashboard/{window}')"},
        *dashboard_inner(snap, count, "poll", window))
...
def header_static(current_window):
    ...
    return h.header({"class": "pg-header spread", "id": "page-header"},
        h.a({"href": "/", "class": "row logo-pill stage glass",
             "style": "text-decoration: none; color: inherit"},
            icon_logo,
            h.strong("Onit"),
            h.span({"id": "uptime",
                    "style": "--type: -2; --fg: -0.5; margin-inline-start: 0.5em"},
                   "—")),
        h.div({"class": "row"},
            ...
            h.button({
                ...
                "data-on:click":
                    "$type = $type === 'sm' ? 'md' : ($type === 'md' ? 'lg' : 'sm'), "
                    "document.body.setAttribute('data-ui-type', $type)",
            }, icon_font)))


def uptime_patch(snap):
    return h.span({"id": "uptime",
                   "style": "--type: -2; --fg: -0.5; margin-inline-start: 0.5em"},
                  f"up {fmt_uptime(snap['uptime_s'])}")


def footer_status(snap, count, mode):
    return h.footer({"class": "pg-footer spread", "id": "status-strip"},
        h.div({"class": "row"},
            h.span({"class": f"tag {'suc' if mode == 'live' else 'inf'}"}, mode),
            h.span(f"{count} viewer{'s' if count != 1 else ''}"),
            h.span({"style": "--fg: -0.5"}, "·"),
            h.span({"class": "mono"}, f"sampler {snap['sampler_fps']:.0f}Hz"),
            h.span({"class": "mono"}, f"tick {snap['sampler_tick_ms']:.1f}ms"),
...
def initial_footer():
    return h.footer({"class": "pg-footer spread", "id": "status-strip"},
        h.div({"class": "row"},
            h.span({"style": "--fg: -0.5"}, "connecting…"),
            h.span({"style": "--fg: -0.5"}, "· client "),
            h.span({"id": "client-fps", "class": "mono",
                    "data-ignore-morph": ""}, "—"),
            h.span({"style": "--fg: -0.5"}, "fps")),
...
def banner():
    return h.section(
        {"class": "pg-banner lcr wrn card stage", "id": "page-banner"},
...
def page(window):
    return h.html({"data-ui-theme": "dark", "id": "html"},
        h.head(
            ...
            h.style(PAGE_STYLE)),
        h.body({"class": "page stage", "id": "body",
                "data-signals": '{"hue":0}'},
            banner(),
            header_static(window),
            h.main({"class": "pg-main", "id": "page-main"},
                h.div({"id": "dashboard",
                       "data-init": f"@get('/dashboard/{window}')"},
                    h.div({"style": "text-align: center; --fg: -0.5"},
                          "connecting…"))),
            initial_footer(),
...
def render_update(snap, count, mode, window, interval_ms=None):
    if mode == "live":
        dashboard = dashboard_live(snap, count, window)
    else:
        dashboard = dashboard_poll(snap, count, interval_ms, window)
    status = footer_status(snap, count, mode)
    uptime = uptime_patch(snap)
    return h_render(dashboard) + h_render(status) + h_render(uptime)
...
def make_dashboard_handler(window):
    def handler(req):
        resource = f"dashboard-{window}"
        if not live.should_be_live(resource):
            snap = snapshot(window)
            count = live.count(resource)
            interval = live.poll_interval_ms(resource)
            yield sse_event_patch(render_update(snap, count, "poll", window, interval))
            return

        with live.join(resource):
            count = live.count(resource)
            yield sse_event_patch(render_update(snapshot(window), count, "live", window))
            while True:
...
for label, _ in WINDOWS:
    ...
    ROUTES.append(("GET", f"/dashboard/{label}", make_dashboard_handler(label)))
